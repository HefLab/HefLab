# Puzzle Builder Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Claude Code skill (`build-puzzle`) that guides the game creator through a 5-phase conversational workflow to build new daily puzzle entries — concept → parallel research → flagged review → approval → write to puzzles.js + commit.

**Architecture:** The entire implementation is a single skill prompt file (`SKILL.md`) that Claude Code reads when the skill is invoked. The file contains complete instructions for all 5 phases, including how to dispatch parallel research agents, handle flagged items, manipulate `puzzles.js` as text, and commit the result. A backup copy is also saved to the project repo for version control.

**Tech Stack:** Claude Code skill system (Markdown prompt file), Agent tool for parallel research subagents, Bash for git operations.

**Spec:** `docs/superpowers/specs/2026-03-22-puzzle-builder-skill-design.md`
...
| Action | File | Purpose |
|--------|------|---------|
| **Create** | `~/.claude/skills/build-puzzle/SKILL.md` | The live skill — Claude Code reads this when `/build-puzzle` is invoked |
| **Create** | `docs/skills/build-puzzle-SKILL.md` | Backup copy in project repo for version control |

No files in `index.html` or `puzzles.js` are modified by this plan — the skill operates on those files at runtime.
...
**Files:**
- Create: `/home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md`

- [ ] **Step 1.1: Create the skill directory**

```bash
mkdir -p /home/lucasmhefner/.claude/skills/build-puzzle
```

- [ ] **Step 1.2: Write `SKILL.md` with the complete workflow**

Create `/home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md` with this exact content:
...
- [ ] **Step 1.3: Verify the file was created correctly**

```bash
ls -la /home/lucasmhefner/.claude/skills/build-puzzle/
head -10 /home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md
```

Expected: file exists, first lines show the YAML frontmatter starting with `---` and `name: build-puzzle`.

- [ ] **Step 1.4: Verify the frontmatter is valid YAML**

```bash
python3 -c "
import re
with open('/home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md') as f:
    content = f.read()
match = re.match(r'^---\n(.*?)\n---', content, re.DOTALL)
if match:
...
Expected output: prints the frontmatter block with `name:` and `description:` fields, ends with `OK`.

---

## Task 2: Save backup to project repo

**Files:**
- Create: `/home/lucasmhefner/MOSTLY BALL/docs/skills/build-puzzle-SKILL.md`

- [ ] **Step 2.1: Create the docs/skills directory**

```bash
mkdir -p "/home/lucasmhefner/MOSTLY BALL/docs/skills"
```

- [ ] **Step 2.2: Copy the skill file into the project**

```bash
cp /home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md \
   "/home/lucasmhefner/MOSTLY BALL/docs/skills/build-puzzle-SKILL.md"
```

- [ ] **Step 2.3: Verify the copy matches**

```bash
diff /home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md \
     "/home/lucasmhefner/MOSTLY BALL/docs/skills/build-puzzle-SKILL.md"
```

Expected: no output (files are identical).

- [ ] **Step 2.4: Commit the backup**

```bash
cd "/home/lucasmhefner/MOSTLY BALL"
git add docs/skills/build-puzzle-SKILL.md
git commit -m "Add build-puzzle skill backup to project repo"
```

---

## Task 3: Smoke test — verify skill is discoverable

This task verifies the skill file is structurally correct and would be picked up by Claude Code's skill detection.

- [ ] **Step 3.1: Confirm skill directory is in the right location**

```bash
ls /home/lucasmhefner/.claude/skills/
```

Expected: `build-puzzle` appears in the list alongside `web-ui-dev`, `game-research-assistant`, and `research-verification-agent`.

- [ ] **Step 3.2: Confirm SKILL.md has all required phases**

```bash
grep -n "## Phase\|## Setup\|## Quick Reference" \
  /home/lucasmhefner/.claude/skills/build-puzzle/SKILL.md
```

Expected output (line numbers may vary):
```
N: ## Setup
N: ## Phase 1: Concept (5 Questions, One at a Time)
N: ## Phase 2: Research (4 Parallel Agents)
N: ## Phase 3: Flagged Items Review
N: ## Phase 4: Final Approval
N: ## Phase 5: Write and Commit
N: ## Quick Reference: answerPool Key Map
```

- [ ] **Step 3.3: Confirm the description field is present and non-empty**
...
if desc:
    ...
    print(desc.group(1).strip()[:120])
    print('OK')
else:
    print('ERROR: description field missing or malformed')
"
```

Expected: prints the first ~120 chars of the description, ends with `OK`.

- [ ] **Step 3.4: Commit task 3 verification (no file changes needed — this is a check-only task)**

If steps 3.1–3.3 all pass, the implementation is complete. No additional commit needed for this task.

---

## Notes for implementer

- The skill file is a **prompt**, not code. There are no unit tests in the traditional sense — correctness is verified by structure checks (Tasks 1 and 3) and by using the skill in a real session.
- The skill lives at `~/.claude/skills/` (user profile), not in the project repo. The backup in `docs/skills/` is for version control only — Claude Code reads from `~/.claude/skills/`.
- If the skill description needs to be updated to improve trigger detection, edit `~/.claude/skills/build-puzzle/SKILL.md` and update the backup copy in `docs/skills/` to match.
- The skill writes to `puzzles.js` at runtime (Phase 5). No changes to `index.html` are needed.
